#!/usr/bin/env python
"""Train a self-supervised network with Poisson PURE.

The Poisson paper's denoising experiment uses ``gamma=4``. In DeepInv's scaled
Poisson convention this is ``gain=1/gamma`` and the loss is
``SurePoissonLoss(gain=gain)``.
"""
...
def main():
    ...
    losses = [dinv.loss.SurePoissonLoss(gain=gain, tau=cfg["training"].get("tau", 1e-4))]
    optimizer = torch.optim.AdamW(bundle.model.parameters(), lr=cfg["training"].get("lr", 5e-4))
    scheduler = torch.optim.lr_scheduler.StepLR(
        optimizer,
        step_size=max(1, int(cfg["training"].get("epochs", 400) * 0.8)),
    )

    save_path = bundle.output_dir / "checkpoints" / "pure"
    ...
    trainer = dinv.Trainer(
        ...
        ckp_interval=cfg["training"].get("ckp_interval", 50),
        eval_interval=cfg["training"].get("eval_interval", 20),
